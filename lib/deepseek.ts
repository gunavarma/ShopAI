export class DeepSeekService {
  ...
  private static readonly BASE_URL = 'https://api.deepseek.com/v1/chat/completions';
  private static isQuotaExceededFlag = false;
  ...
  static getQuotaResetTime(): number | null {
  ...
  static async generateResponse(prompt: string): Promise<string> {
    ...
    try {
      ...
      throw new Error('Failed to generate AI response from DeepSeek');
    }
  }
...
`;
      ...
      try {
        ...
        console.error('Failed to parse image URLs from DeepSeek:', parseError);
      }
      
      // Fallback images based on category
      return this.getFallbackImages(category);
    } catch (error) {
      console.error('Error getting related images from DeepSeek:', error);
      return this.getFallbackImages(category);
    }
  }
  ...
  static async analyzeProductQuery(userQuery: string, availableProducts: any[]): Promise<{
    intent: string;
    category?: string;
...
`;
    ...
    try {
      ...
      console.error('Error analyzing query with DeepSeek:', error);
      return {
        intent: 'search',
        sentiment: 'neutral',
        confidence: 0.5
  ...
  static async generateProductRecommendation(
    ...
    const prompt = `
You are ShopWhiz, an AI shopping assistant. Generate a helpful, conversational response for this user query.
...
`;
    ...
    try {
      ...
      console.error('Error generating recommendation with DeepSeek:', error);
  ...
  static async generateQuizRecommendation(
    ...
    const prompt = `
Based on the user's quiz responses, generate a personalized product recommendation.
...
`;
    ...
    try {
      ...
      console.error('Error generating quiz recommendation with DeepSeek:', error);
  ...
  static async generateProductComparison(products: any[]): Promise<string> {
    ...
    const prompt = `
Compare these products and provide a helpful analysis:
...
`;
    ...
    try {
      ...
      console.error('Error generating comparison with DeepSeek:', error);
