      ...
      <div className={`flex flex-col gap-2 max-w-[85%] ${isUser ? 'items-end' : 'items-start'}`}>
        ...
        {!isUser && message.needsMoreInfo && message.category && (
          <motion.div
            ...
            transition={{ delay: useStreamingText ? 2 : 0.4 }}
            className="w-full"
          >
            <BrandPriceSelector
        ...
        {!isUser && message.clarifyingQuestions && message.clarifyingQuestions.length > 0 && !message.needsMoreInfo && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: useStreamingText ? 2.5 : 0.4 }}
            ...
            <div className="space-y-2">
              {message.clarifyingQuestions.map((question, index) => (
                <motion.button
                  ...
                  transition={{ delay: (useStreamingText ? 3 : 0.5) + index * 0.1 }}
        ...
        {!isUser && message.brandSuggestions && message.brandSuggestions.length > 0 && !message.needsMoreInfo && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: useStreamingText ? 3 : 0.5 }}
            ...
            <div className="flex flex-wrap gap-2">
              {message.brandSuggestions.map((brand, index) => (
                <motion.button
                  ...
                  transition={{ delay: (useStreamingText ? 3.2 : 0.6) + index * 0.05 }}
        ...
        {!isUser && message.suggestedActions && message.suggestedActions.length > 0 && !message.needsMoreInfo && (
          <motion.div
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: useStreamingText ? 3.5 : 0.5 }}
          >
            <SuggestedActions
